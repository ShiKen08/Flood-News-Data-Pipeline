...
def main() -> None:
    ...
    # so each batch job loads only its ~1k rows, not the full 13k.
    _pq_filters = None
    ...
    # Assign scores back by index — avoids a merge on doc_id which may not be unique
    # and would cause a cartesian explosion if any doc_id is duplicated.
    df["model_flood_prob"]       = np.nan
    df["model_is_event_article"] = False
    df.loc[df_with_text.index, "model_flood_prob"]       = probs_arr
    df.loc[df_with_text.index, "model_is_event_article"] = probs_arr >= args.threshold
    del df_with_text, probs_arr
    gc.collect()
